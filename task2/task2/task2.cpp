...
public:
    ...
    Vector(double value) { data[0] = data[1] = data[2] = value; objectCount++; }
    Vector(double x, double y, double z) { data[0] = x; data[1] = y; data[2] = z; objectCount++; }
    Vector(const double* arr) { if (arr) for (int i = 0; i < 3; i++) data[i] = arr[i]; objectCount++; }
    ~Vector() { objectCount--; }
    ...
    Vector divide(short divisor) const { return divisor == 0 ? *this : Vector(data[0] / divisor, data[1] / divisor, data[2] / divisor); }
